...
class ExertUvaAlgorithm:
    ...
    @staticmethod
    def get_some_info(df: pd.DataFrame) -> Tuple[dict, str, list, Optional[list]]:
        """
        ...
        if not ExertUvaAlgorithm.validate_grpby_list(grpby_list):
            raise ValueError("Invalid grpby_list. Must include at least one of 'PRODUCT_ID', 'OPER_NO', 'EQP_NAME', 'PRODG1', 'TOOL_NAME'.")
    ...
    def run(df_info: pd.DataFrame, df1: pyspark.sql.dataframe):
        try:
            parse_dict, request_id, grpby_list, merge_operno = ExertUvaAlgorithm.get_some_info(df_info)
            # todo1: 将parse_dict传给解析SQL的函数, 拼接SQL获取数据

            # 1. Station merge and data preprocessing
            df_merge_operno = PreprocessForUvaData.integrate_operno(df=df1, merge_operno_list=merge_operno)
            ...
            df_run = PreprocessForUvaData.pre_process(df1)
...
if __name__ == '__main__':
    ...
    spark = SparkSession.builder \
        ...
        .config('spark.driver.host', '192.168.236.1') \
