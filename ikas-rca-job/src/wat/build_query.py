...
def build_wat_query(select_condition_dict: dict, properties_config: dict) -> str:
    table_name = f"{properties_config['doris_db']}.{properties_config['doris_wat_wafer_summary_table']}"
    uploaded_wafer_table_name = f"{properties_config['doris_db']}.{properties_config['doris_uploaded_wafer_table']}"
    wat_key_mapping_table_name = f"{properties_config['doris_db']}.{properties_config['doris_wat_key_mapping_table_name']}"
    # 查询条件转sql,并打标签，label '0': good wafer, '1': bad wafer
    filter_sql_list = []
    for keyword, value in select_condition_dict.items():
        if keyword in ["productId", "prodg1", "parametricName"]:
    ...
    key_mapping_sql = ""
    item_type = select_condition_dict.get("itemType")
    ...
    if upload_id is not None and len(upload_id) > 0:
        ...
        on d1.WAFER_ID = d2.name
            where {filter_sql_concat} and d2.UPLOAD_ID = '{upload_id}' {key_mapping_sql}"""
        else:
            select_sql = f"""SELECT *, {case_when_statment} from {table_name} d1  
            join {uploaded_wafer_table_name} d2
        on d1.WAFER_ID = d2.name where d2.UPLOAD_ID = '{upload_id}' {key_mapping_sql} """
