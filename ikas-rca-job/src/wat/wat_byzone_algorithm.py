...
class FitWatModelByZone:
    def __init__(self,
                 df: pyspark.sql.dataframe,
                 grpby_list: list[str],
                 wat_info: Dict[str, Dict[str, List[str]]],
    ...
    def get_pivot_table(df, grpby_list, columns_to_process, missing_value_threshold):
        df = FitWatModelByZone.process_missing_values(df, columns_to_process, missing_value_threshold)
        ...
        pivot_result = FitWatModelByZone.process_missing_values(pivot_result, pivot_result.columns,
    ...
    @staticmethod
    def fit_pca_model(df: pyspark.sql.dataframe, grpby_list, wat_info,
        ...
        def get_model_result(df_run):
            ...
            pivot_result = FitWatModelByZone.get_pivot_table(df=side_with_features_all,
                                                             grpby_list=grpby_list,
            ...
            if min(x_train.shape) > 2:
                res_top_select = FitWatModelByZone.construct_features_when_satisfy_pca(x_train=x_train)
                return res_top_select
            else:
                res_top_select = FitWatModelByZone.construct_features_when_not_satisfied(x_train=x_train)
                return res_top_select
...
class ExertWatByZone:
    ...
    def fit_by_zone_model(df: pyspark.sql.dataframe,
                          ...
                          grpby_list=None,
                          model='pca'):
        wat_info = mode_info.get("WAT")
        length = len(wat_info)
        if length < 2:
            msg = "请至少指定两类不同的Zone"
            raise RCABaseException(msg)
        site_columns = ExertWatByZone.get_site_vals(wat_info)
        ...
        res = FitWatModelByZone(df=df_preprocess,
                                grpby_list=grpby_list,
                                wat_info=wat_info,
...
if __name__ == "__main__":
    ...
    time1 = datetime.now()
    final_res_ = ExertWatByZone.fit_by_zone_model(df=df_spark,
