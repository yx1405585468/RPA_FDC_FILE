    ...
    def run(self) -> pyspark.sql.dataframe:
        ...
        if grps_large.isEmpty():
            ...
            if grps_less.isEmpty():
                ...
                # if self.grpby_list == ['OPE_NO']:
                #     msg = "分组后的数据, 没有组合满足条件good >= 1且bad >= 1, 无法进行分析."
                # else:
        ...
        def get_model_result(df_run):
            ...
            if x_train.shape[1] > 1 and y_train['label'].nunique() > 1:
                ...
                try:
                    ...
                    return FitWatModelByWafer.construct_features_when_not_satisfied(df_run, pivot_result, x_train, grpby_list, 'classification')
...
class ExertWatByWafer:
    ...
    def fit_by_wafer_model(df: pyspark.sql.dataframe,
        ...
        # res_pandas.to_csv("res_pandas_by_wafer.csv")
