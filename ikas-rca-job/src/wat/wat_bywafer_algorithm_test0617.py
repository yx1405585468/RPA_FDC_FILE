    ...
    def fit_pca_model(df: pyspark.sql.dataframe, grpby_list, columns_to_process,
        ...
        def get_model_result(df_run):
            ...
            if min(x_train.shape) > 2:
                ...
                res_top_select = FitWatModelByWafer.construct_features_when_not_satisfied(df_run, x_train, grpby_list, 'pca')
                return res_top_select
        ...
        def get_model_result(df_run):
            ...
            if x_train.shape[1] > 1 and y_train['label'].nunique() > 1:
                ...
                try:
                    ...
                    return FitWatModelByWafer.construct_features_when_not_satisfied(df_run, x_train, grpby_list, 'classification')
                ...
                res_top_select = FitWatModelByWafer.construct_features_when_not_satisfied(df_run, x_train, grpby_list, 'classification')
...
class ExertWatByWafer:
    ...
    def fit_by_wafer_model(df: pyspark.sql.dataframe,
        ...
        add_parametric_stats_df, df_preprocess = DataPreprocessorForWat(df=df,
                                                                        grpby_list=grpby_list,
        ...
        if big_or_small == 'big':
            ...
            final_res = SplitWatModelResults.add_certain_column(df=final_res, request_id=request_id)
            return final_res
