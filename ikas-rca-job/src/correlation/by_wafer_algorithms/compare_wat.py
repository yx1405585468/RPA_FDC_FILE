...
from src.correlation.common_process.data_preprocessing import ProcessBaseDFByWAFER,PrePareResultTableToCommonSchema
...
class PreprocessForWatData(object):
    ...
    def integrate_columns(df: pyspark.sql.dataframe,
        ...
        """
        df_merged = PreprocessForWatData.integrate_single_column(df, merge_operno_list, 'OPE_NO')
        df_merged = PreprocessForWatData.integrate_single_column(df_merged, merge_prodg1_list, 'PRODG1')
        df_merged = PreprocessForWatData.integrate_single_column(df_merged, merge_product_list, 'PRODUCT_ID')
        return df_merged
...
class CorrCompareWatByWaferAlgorithm(ProcessBaseDFByWAFER, PreprocessForWatData):
    ...
    def run(base_df: pyspark.sql.DataFrame, source_df: pyspark.sql.DataFrame, request_id: str,
        ...
        return CorrCompareWatByWaferAlgorithm.___run_with_kwargs__(base_df, source_df, request_id,
                                                                   **select_config_dict)

    @staticmethod
    def ___run_with_kwargs__(base_df: pyspark.sql.dataframe,
        ...
        # 异常表征 df处理
        base_df, outlier_parametric_name = ProcessBaseDFByWAFER.process_by_wafer(df=base_df)



        # source_df wat 数据预处理
        # 筛选列
        columns_list = ['WAFER_ID', 'PRODG1', 'OPE_NO', 'PRODUCT_ID', 'PARAMETRIC_NAME', 'AVERAGE', 'MAX_VAL',
                        'MEDIAN', 'MIN_VAL', 'STD_DEV']

        # 转数值类型列
        convert_to_numeric_list = ['AVERAGE', 'MAX_VAL', 'MEDIAN', 'MIN_VAL', 'STD_DEV']

        df_processed = PreprocessForWatData(df=source_df,
