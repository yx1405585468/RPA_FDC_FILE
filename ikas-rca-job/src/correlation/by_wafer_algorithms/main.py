...
from src.correlation.by_wafer_algorithms.compare_inline import (
    CorrCompareInlineByWaferAlgorithm,
)
from src.correlation.by_wafer_algorithms.compare_uva import (
    CorrCompareUvaByWaferAlgorithm,
)
from src.correlation.by_wafer_algorithms.compare_wat import (
    CorrCompareWatByWaferAlgorithm,
)
from src.correlation.by_wafer_algorithms.compare_process import (
    CorrCompareProcessByWaferAlgorithm,
)
import pyspark
...
from src.correlation.by_wafer_algorithms.compare_qtime import (
    CorrCompareQTimeByWaferAlgorithm,
)
...
class CorrCompareByWaferAlgorithm(object):
    algo_dispatch_mapt = {
        ...
        "qtime": CorrCompareQTimeByWaferAlgorithm,
    }
    ...
    def run(self):
        ...
        for key, value in self.config_dict.items():
            ...
            elif key in self.algo_dispatch_mapt.keys():
...
if __name__ == "__main__":
    ...
    def test_single_uva(spark):
        ...
        base_df_spark = (
            ps.from_pandas(base_df)
            .to_spark()
            .select("WAFER_ID", col("RESULT").alias("ParName"))
        )
        # base_df_spark.show()
        ...
        final_res_ = CorrCompareByWaferAlgorithm(
            base_df=base_df_spark, source_df=df_spark, config_dict=json_loads_dict
        ).run()
    ...
    def test_single_inline(spark):
        ...
        base_df_spark = ps.from_pandas(
            base_df[["WAFER_ID", "AVERAGE"]].rename(columns={"AVERAGE": "ParName"})
        ).to_spark()
        base_df_spark.show()
        #
        ...
        final_result = CorrCompareInlineByWaferAlgorithm.run(
