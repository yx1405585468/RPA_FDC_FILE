...
from src.correlation.common_process.data_preprocessing import PrePareResultTableToCommonSchema,ProcessBaseDFBySite,MergeAllDataSourceToResultTable
...
class CorrCompareWatBySiteAlgorithm(PreprocessForWatData, CorrelationDetectAlgorithm):
    ...
    def run(base_df: pyspark.sql.DataFrame, source_df: pyspark.sql.DataFrame, request_id: str,
        ...
        return CorrCompareWatBySiteAlgorithm.__run_with_kwargs__(base_df, source_df, request_id,
                                                                     **select_config_dict)
    ...
    def __run_with_kwargs__(base_df: pyspark.sql.DataFrame,
        ...
        def deal_each_column(result: pyspark.sql.DataFrame, stat_col: str, base_site_columns: List[str],
            ...
            result = (
                result.select(group_by_list + ["PARAMETRIC_NAME", "COUNT"] + base_site_columns + stat_source_site_columns)
        ...
        df_processed = PreprocessForWatData(source_df,
        ...
        for col_name in common_site_columns:
            result_all_stat_cols = result_all_stat_cols.withColumn(f'BASE_{col_name}', lit(base_df_pandas[col_name].values[0]).cast('float'))
            base_site_cols.append(f'BASE_{col_name}')

        # 计算bysite 的指标
        # 对bywafer 多个聚合值，一个一个处理，最后拼接
        result_list = [deal_each_column(result_all_stat_cols, stat_col, base_site_columns=base_site_cols,source_site_columns=common_site_columns)
