...
from pyspark.sql.functions import max, countDistinct, when, lit, pandas_udf, PandasUDFType, monotonically_increasing_id, \
    split, collect_set, concat_ws, col, count, countDistinct, udf, mean, first,row_number
...
class ProcessBaseDFByWAFER(object):
    ...
    @staticmethod
    def process_by_wafer(df: pyspark.sql.DataFrame,first_column_name:str="WAFER_ID"):
        """
        df 字段
        -----------------
        WAFER_ID, | value
...
class MergeAllDataSourceToResultTable(object):
    ...
    def run(df_list: List[Optional[pyspark.sql.DataFrame]]):
        ...
        if len(not_empty_df_list) == 0:
            raise RCABaseException("所有数据源结果表都为空！,请检查每个分组相关性两列都非空的数目是不是为0")
        ...
        result = (result
                  ...
                  .withColumn(
                  "INDEX_NO",
                  row_number().over(Window.orderBy(col("WEIGHT").desc(), col("COUNT").desc())
                  )))
    ...
    def integrate_columns(df: pyspark.sql.dataframe,
        ...
        for key, value in kwargs.items():
            if key in df_columns:
                df = MergeOneColumnMultiValuesIntoNewOne.integrate_single_column(df, value, key)
            else:
                raise RCABaseException("The column: {} does not exist in the DataFrame".format(key))
        return df
    @staticmethod
