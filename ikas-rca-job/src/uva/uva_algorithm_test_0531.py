...
from pyspark.sql.functions import max, col, countDistinct, when, lit, pandas_udf, PandasUDFType, monotonically_increasing_id, split, collect_set, concat_ws
...
class FitModelForUvaData:
    ...
    def fit_classification_model(df: pyspark.sql.DataFrame, grpby_list: List[str], model) -> pyspark.sql.DataFrame:
        ...
        struct_fields.extend([StructField("bad_wafer", IntegerType(), True),
                              ...
                              StructField("importance_type", StringType(), True),
        ...
        def get_model_result(df_run: pd.DataFrame) -> pd.DataFrame:
            ...
            if x_train.shape[1] > 1 and y_train['label'].nunique() > 1:
                ...
                perm_importance = permutation_importance(best_est, x_train.values, y_train.values.ravel(),
                                                         n_repeats=50, random_state=42, n_jobs=-1)
                perm_importance_res = pd.DataFrame(
                    {'features': x_train.columns, 'importance': perm_importance.importances_mean})

                # Combine the importance results
                small_importance_res['importance_type'] = 'impurity_based'
                perm_importance_res['importance_type'] = 'permutation'
                combined_importance_res = pd.concat([small_importance_res, perm_importance_res])
                ...
                sample_res_dict.update({col_: df_run[col_].values[0] for col_ in grpby_list})
                small_sample_res = pd.DataFrame(sample_res_dict, index=[0])
                res_top_select = pd.concat([combined_importance_res, small_sample_res])
                return res_top_select
            else:
...
class ExertUvaAlgorithm:
    ...
    def fit_uva_model(df: pyspark.sql.dataframe,
        ...
        # train_data_pandas.to_csv("train_data_pandas.csv")
