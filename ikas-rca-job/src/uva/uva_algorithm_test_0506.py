...
class PreprocessForUvaData:
    ...
    #                           StructField("BAD_NUM", FloatType(), True)])
    #     schema_all = StructType(struct_fields)
    #
    #     @pandas_udf(returnType=schema_all, functionType=PandasUDFType.GROUPED_MAP)
    #     def get_result(df_run) -> pd.DataFrame:
    #         grpby_list_extend = grpby_list + ['PARAM', 'STEP']
    #         feature_stats = df_run.groupby(grpby_list_extend)['STATS'].unique().reset_index()
    ...
    #             PARAMETRIC_NAME=lambda x: x['PARAM'] + str('#') + x['STEP']).drop('STEP', axis=1)
    ...
    def add_feature_stats_within_groups(df_integrate: pyspark.sql.dataframe,
        ...
        print("split_params_for_df_integrate:", split_params_for_df_integrate.count())
        split_params_for_df_integrate.show(10)
        ...
        unique_params_within_groups.show(10)
        return unique_params_within_groups
    ...
    def run(self):
        ...
        add_parametric_stats_df = self.add_feature_stats_within_groups(df_integrate=df_integrate_columns, grpby_list=self.grpby_list)
        ...
        def get_model_result(df_run: pd.DataFrame) -> pd.DataFrame:
            ...
            if x_train.shape[1] > 1 and y_train['label'].nunique() > 1:
                ...
                try:
                    ...
                    return FitModelForUvaData.construct_features_when_not_satisfied(df_run, df_pivot, x_train, grpby_list, 'classification')
...
class GetFinalResultsForUvaData:
        ...
        def get_result(model_results: pd.DataFrame) -> pd.DataFrame:
            ...
            feature_importance_groupby = (
                feature_importance_res_split.groupby(
                    grpby_list + ['PARAMETRIC_NAME', 'STEP', 'algorithm_satisfied', 'x_train_shape'])['importance'].sum().reset_index())
    ...
    def run(self):
        if self.big_or_small == 'big':
            ...
            # res_all_pandas = res_all.toPandas()
            # res_all_pandas.to_csv("res_all_pandas.csv")
            ...
            f_res.show()
            print(f_res.select("PARAMETRIC_NAME").collect()[0][0])
            # f_res_pandas_pca = f_res.toPandas()
            # f_res_pandas_pca.to_csv("f_res_pandas_pca.csv")

            res_all = self.get_final_results_small_sample(f_res=f_res, grpby_list=self.grpby_list)
            print("res_all:", res_all.count())
            res_all.show()
            print(res_all.select("PARAMETRIC_NAME").collect()[0][0])
            # res_all_pandas_pca = res_all.toPandas()
            # res_all_pandas_pca.to_csv("res_all_pandas_pca.csv")
        ...
        missing_rows = self.add_parametric_stats_df.join(res_all, on=self.grpby_list + ['PARAMETRIC_NAME', 'STATS'], how='left_anti')
...
class ExertUvaAlgorithm:
    ...
    def fit_uva_model(df: pyspark.sql.dataframe,
        ...
        print("最后算法结果是：", result.count())
        print(result.select("features").collect()[0][0])
        result.show()
        # result_pandas = result.toPandas()
        # result_pandas.to_csv("result_pandas.csv")
        ...
        print("写回数据库的结果是：", final_res.count())
        final_res.show()
        return final_res
