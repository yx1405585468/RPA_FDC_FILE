...
class DataPreprocessorForInline:
    ...
    def add_feature_stats_within_groups(df_integrate: pyspark.sql.dataframe, grpby_list) -> pyspark.sql.dataframe:
        unique_labels = [row['label'] for row in df_integrate.select('label').distinct().collect()]
        agg_exprs = [
...
class GetTrainDataForInline:
    ...
    def run(self) -> pyspark.sql.dataframe:
        unique_labels = [row['label'] for row in self.df_run.select('label').distinct().collect()]
    ...
    def fit_pca_model(df: pyspark.sql.dataframe, grpby_list, columns_to_process,
        ...
        def get_model_result(df_run):
            ...
            if min(x_train.shape) > 2:
                ...
                res_top_select = FitInlineModelByWafer.construct_features_when_not_satisfied(df_run, x_train, grpby_list, 'pca')
                return res_top_select
        ...
        def get_model_result(df_run):
            ...
            if x_train.shape[1] > 1 and y_train['label'].nunique() > 1:
                ...
                try:
                    ...
                    return FitInlineModelByWafer.construct_features_when_not_satisfied(df_run, x_train, grpby_list, 'classification')
                ...
                res_top_select = FitInlineModelByWafer.construct_features_when_not_satisfied(df_run, x_train, grpby_list, 'classification')
                return res_top_select
...
class SplitInlineModelResults:
    ...
    def split_calculate_features_big_sample(df: pyspark.sql.dataframe, grpby_list: List[str]) -> pyspark.sql.dataframe:
        ...
        def get_result(model_results: pd.DataFrame) -> pd.DataFrame:
            ...
            feature_importance_res_split = SplitInlineModelResults.get_split_features(df=feature_importance_table,
                                                                                      grpby_list=grpby_list)
            feature_importance_groupby = (feature_importance_res_split.groupby(grpby_list + ['PARAMETRIC_NAME'])[
    ...
    @staticmethod
    def get_final_results_big_sample(s_res: pyspark.sql.dataframe, f_res: pyspark.sql.dataframe,
    ...
    @staticmethod
    def split_calculate_features_small_sample(df: pyspark.sql.dataframe, grpby_list: List[str],
                                              by) -> pyspark.sql.dataframe:
        struct_fields = [StructField(col_, StringType(), True) for col_ in grpby_list]
        struct_fields.extend([StructField("PARAMETRIC_NAME", StringType(), True),
                              StructField("importance", FloatType(), True),
                              StructField("algorithm_satisfied", StringType(), True),
        ...
        def get_model_result(df_run):
            ...
            split_table = SplitInlineModelResults.get_split_features(df=feature_importance_table, grpby_list=grpby_list)
    ...
    def add_certain_column(df: pyspark.sql.dataframe, request_id: str) -> pyspark.sql.dataframe:
        ...
        df = df.withColumn("GOOD_NUM", df["GOOD_NUM"].cast(FloatType())).withColumn("BAD_NUM", df["BAD_NUM"].cast(FloatType()))

        df = df.withColumn("WEIGHT_PERCENT", col("WEIGHT") * 100).withColumn("REQUEST_ID", lit(request_id))
...
class ExertInlineByWafer:
    ...
    def fit_by_wafer_model(df: pyspark.sql.dataframe,
        ...
        add_parametric_stats_df, df_preprocess = DataPreprocessorForInline(df=df,
                                                                           grpby_list=grpby_list,
        ...
        common_res, train_data, big_or_small = GetTrainDataForInline(df=df_preprocess, grpby_list=grpby_list).run()
        ...
        if big_or_small == 'big':
            ...
            res = FitInlineModelByWafer.fit_classification_model(df=train_data,
                                                                 grpby_list=grpby_list,
            ...
            res = FitInlineModelByWafer.fit_pca_model(df=train_data,
                                                      grpby_list=grpby_list,
                                                      columns_to_process=['AVERAGE', 'MAX_VAL', 'MEDIAN',
                                                                          'MIN_VAL', 'STD_DEV'],
            ...
            final_res = SplitInlineModelResults.add_certain_column(df=final_res, request_id=request_id)
            return final_res
...
if __name__ == "__main__":
    ...
    df_pandas = pd.read_csv(
        "D:/Jupyterfiles/晶合MVAFDC_general开发/MVAanlysisDevelop/inline_algorithm/inline_case5_label_three_class.csv")
