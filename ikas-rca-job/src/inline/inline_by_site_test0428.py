...
from pyspark.sql.functions import pandas_udf, PandasUDFType, lit, col, when, sum as spark_sum, \
    monotonically_increasing_id, collect_set, explode, countDistinct
...
class DataPreprocessorForInline:
    ...
    def run(self) -> pyspark.sql.dataframe:
        ...
        # unique_params_within_groups = self.extract_unique_params_within_groups(df=df_integrate, grpby_list=self.grpby_list)

        unique_params_within_groups = (df_integrate.groupBy(self.grpby_list + ['PARAMETRIC_NAME'])
        ...
        # df_integrate_pandas = df_integrate.toPandas()
        # df_integrate_pandas.to_csv("df_integrate_pandas.csv")
        #
        # unique_params_within_groups_pandas = unique_params_within_groups.toPandas()
        # unique_params_within_groups_pandas.to_csv("unique_params_within_groups_pandas.csv")
        #
        # df_preprocess_pandas = df_preprocess.toPandas()
        # df_preprocess_pandas.to_csv("df_preprocess_pandas.csv")
        return grps_all, unique_params_within_groups, df_preprocess
...
class SplitInlineModelResults:
    ...
    def run(self):
        ...
        print("第一次split_res:", split_res.count())
        split_res.show(50)
        # split_res_pandas = split_res.toPandas()
        # split_res_pandas.to_csv("split_res_pandas.csv")

        split_res = split_res.join(self.unique_params_within_groups,
                                   on=self.grpby_list + ['PARAMETRIC_NAME'],
                                   how='left')
        print("第二次split_res:", split_res.count())
        split_res.show(50)
        ...
        #     self.grps_all.select(self.grpby_list + ["GOOD_NUM"]),
        #     on=self.grpby_list, how='left')

        final_res = self.add_certain_column(df=split_res_update_missing_features, request_id=self.request_id)
        return final_res
...
class ExertInlineBySite:
    ...
    def fit_by_site_model(df: pyspark.sql.dataframe,
        ...
        grps_all, unique_params_within_groups, df_preprocess = DataPreprocessorForInline(df=df,
                                                                                         grpby_list=grpby_list,
        ...
        final_res_pandas.to_csv("final_res_pandas_site.csv")
        return final_res
